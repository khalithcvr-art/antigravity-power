...
export const CostEstimatorModal: React.FC<CostEstimatorModalProps> = ({ isOpen, onClose, isArabic = false }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-black/80 backdrop-blur-xl animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-4xl bg-obsidian-900 border border-white/15 rounded-3xl shadow-2xl shadow-black overflow-hidden flex flex-col my-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-y-auto max-h-[75vh]">
                ...
                {JURISDICTIONS.map((j) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] text-emerald-400/90 font-mono mt-1">
                      {isArabic ? 'تبدأ من ' : 'From '} AED {j.baseCost.toLocaleString()}
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-base font-bold text-emerald-400 font-mono">
                  {calcState.visaCount} {isArabic ? 'تأشيرة' : (calcState.visaCount === 1 ? 'Visa' : 'Visas')}
                </span>
              </div>
              <input
                type="range"
                ...
                onChange={(e) => setCalcState(prev => ({ ...prev, visaCount: parseInt(e.target.value) || 0 }))}
                className="w-full accent-emerald-500 bg-obsidian-950 h-2 rounded-lg cursor-pointer"
              />
              <div className="flex justify-between text-[10px] font-mono text-slate-500 mt-1">
                <span>0 {isArabic ? '(ترخيص فقط)' : '(License Only)'}</span>
                <span>3 {isArabic ? 'تأشيرات' : 'Visas'}</span>
                <span>6 {isArabic ? 'تأشيرات' : 'Visas'}</span>
                <span>10+ {isArabic ? 'تأشيرات' : 'Visas'}</span>
              </div>
            </div>

            {/* Step 3: Office Requirement */}
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                >
                  {isArabic ? 'مكتب مرن (توثيق)' : 'Flexi-Desk (Tawtheeq)'}
                </button>
                <button
                  onClick={() => setCalcState(prev => ({ ...prev, officeType: 'dedicated' }))}
                  className={`p-2.5 rounded-xl border text-center text-xs font-medium transition-all ${
                    calcState.officeType === 'dedicated'
                      ? 'bg-emerald-500/10 border-emerald-500 text-emerald-300 font-bold'
                      : 'bg-obsidian-950 border-white/5 text-slate-400 hover:text-white'
                  }`}
                >
                  {isArabic ? 'مكتب خاص' : 'Dedicated Office'}
                </button>
                <button
                  onClick={() => setCalcState(prev => ({ ...prev, officeType: 'warehouse' }))}
                  className={`p-2.5 rounded-xl border text-center text-xs font-medium transition-all ${
                    calcState.officeType === 'warehouse'
                      ? 'bg-emerald-500/10 border-emerald-500 text-emerald-300 font-bold'
                      : 'bg-obsidian-950 border-white/5 text-slate-400 hover:text-white'
                  }`}
                >
                  {isArabic ? 'مستودع / وحدة' : 'Warehouse / Unit'}
                </button>
              </div>
            </div>

            {/* Step 4: Strategic Add-On Toggles */}
            <div>
              <label className="block text-xs font-mono uppercase tracking-wider text-slate-300 mb-2">
                {t.step4Title}
              </label>
              <div className="space-y-2">
                ...
                <label className="flex items-center justify-between p-3 rounded-xl bg-obsidian-950/60 border border-white/5 cursor-pointer hover:border-white/10">
                  ...
                  <span className="text-xs font-mono text-emerald-400 shrink-0">+ AED 4,500</span>
                </label>
          ...
          <div className="lg:col-span-5 p-6 sm:p-8 bg-obsidian-950/90 flex flex-col justify-between space-y-6">
            ...
            {/* Total Highlight */}
            <div className="p-5 rounded-2xl bg-gradient-to-br from-obsidian-900 to-obsidian-850 border border-emerald-500/30">
              ...
              <div className="flex items-baseline space-x-2 rtl:space-x-reverse mb-1">
                <span className="text-3xl sm:text-4xl font-display font-black text-white">
                  AED {breakdown.totalAED.toLocaleString()}
                </span>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full py-3.5 px-4 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-obsidian-950 font-display font-bold text-sm flex items-center justify-center space-x-2 rtl:space-x-reverse transition-all shadow-lg shadow-emerald-500/25 hover:scale-[1.01]"
              >
              ...
              >
                {isArabic ? 'إغلاق ومتابعة تصفح الخدمات' : 'Close & Explore Other Services'}
              </button>
            </div>
