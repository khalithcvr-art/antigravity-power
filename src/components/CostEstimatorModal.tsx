...
export const CostEstimatorModal: React.FC<CostEstimatorModalProps> = ({ isOpen, onClose, isArabic = false }) => {
  ...
  return <AnimatePresence>{isOpen && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" dir={isArabic ? 'rtl' : 'ltr'}>
    ...
    <motion.div role="dialog" aria-modal="true" aria-labelledby="quote-request-title" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative w-full max-w-2xl bg-obsidian-900 border border-white/15 rounded-3xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto text-white">
