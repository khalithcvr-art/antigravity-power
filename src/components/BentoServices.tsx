...
export const BentoServices: React.FC<BentoServicesProps> = ({
  ...
  return (
    <section id="services" className={`py-24 relative bg-obsidian-950/60 border-t border-white/5 ${isArabic ? 'font-arabic' : ''}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <h2 className="text-3xl sm:text-5xl font-display font-bold text-white tracking-tight">
              {mode === 'corporate' ? (
                ...
                isArabic ? (
                  <>
                    واجهات التقنية المالية و{' '}
                    <span className="text-gradient-digital">البنية الرقمية للشركات</span>
                  </>
                ) : (
                  <>
                    FinTech UI/UX &{' '}
                    <span className="text-gradient-digital">Digital Architecture</span>
                  </>
                )
              )}
            </h2>
          ...
          {/* Filter Pills for Corporate Mode */}
          {mode === 'corporate' && (
            <div className="flex items-center space-x-1.5 rtl:space-x-reverse p-1 rounded-xl bg-obsidian-900 border border-white/10 text-xs overflow-x-auto">
              ...
              >
                {isArabic ? 'كافة الخدمات (11)' : 'All 11 Modules'}
              </button>
              <button
              ...
              >
                {isArabic ? 'التأسيس والتراخيص' : 'Formation & Licenses'}
              </button>
              <button
              ...
              >
                {isArabic ? 'العلاقات العامة ووزارة العمل' : 'PRO & MoHRE'}
              </button>
              <button
              ...
              >
                {isArabic ? 'الإقامات والضرائب' : 'Visas & Corporate Tax'}
              </button>
            </div>
          )}
        </div>

        {/* Corporate Mode Bento Grid */}
        {mode === 'corporate' && (
            ...
            {filteredCorporate.map((service, index) => {
              ...
              return (
                <div
                  key={service.id}
                  id={service.anchorId}
                  className={`group relative p-6 sm:p-7 rounded-3xl glass-panel glass-panel-hover flex flex-col justify-between transition-all duration-300 ${
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center space-x-2 rtl:space-x-reverse">
                        {badge && (
                          <span className="px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-[11px] font-mono text-emerald-400 font-semibold">
                            {badge}
                          </span>
                        )}
                    ...
                    <div className="p-3.5 rounded-xl bg-obsidian-950/80 border border-white/5 mb-5 text-xs text-slate-400 leading-relaxed font-mono">
                  ...
                  <div className="pt-4 border-t border-white/5 flex items-center justify-between mt-auto">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-white/5 hover:bg-emerald-500 hover:text-obsidian-950 border border-white/10 hover:border-emerald-400 text-xs font-bold text-slate-200 transition-all flex items-center space-x-2 rtl:space-x-reverse group/btn"
                    >
        ...
        {/* Digital & Creative Mode Bento Grid */}
        {mode === 'digital' && (
              ...
              {DIGITAL_ENGINE_SERVICES.map((pillar) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 group-hover:scale-110 group-hover:bg-cyan-500 group-hover:text-obsidian-950 transition-all duration-300">
                          <IconComponent className="w-6 h-6" />
                        </div>
                        <span className="px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-xs font-mono text-cyan-400 font-semibold">
                          {category}
                        </span>
                      </div>
                    ...
                    <button
                      onClick={() => handleServiceWhatsApp(title)}
                      className="w-full py-3 rounded-xl bg-cyan-500/10 hover:bg-cyan-500 hover:text-obsidian-950 border border-cyan-500/30 text-xs font-bold text-cyan-300 transition-all flex items-center justify-center space-x-2 rtl:space-x-reverse mt-4"
                    >
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-xs font-mono font-bold text-emerald-400">
                            {metric}
                          </span>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-semibold text-slate-200 flex items-center justify-center space-x-1.5 rtl:space-x-reverse transition-colors"
                      >
